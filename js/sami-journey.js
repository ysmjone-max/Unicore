/**
 * LINEA UNICORE: Full-Page Interactive Transition Simulator (v7.0)
 * Immersive Fullscreen Stage, Dynamic Stepper, Live Budget Simulator, Healthcare Triage, Native Speech Audio
 */

(function () {
  'use strict';

  const STORAGE_VISITED_KEY = 'unicore_sim_visited_stations';
  const STORAGE_CHECKS_KEY = 'unicore_sim_checked_tasks';
  ...
  // 12 Simulation Stations Master Database
  const STATIONS = [
    {
      code: '0A',
      name: 'Partenza (Pre-Departure)',
      short: 'Pre-Departure',
      phase: 'PHASE 0 &bull; BEFORE LEAVING (T-MINUS 30 DAYS)',
      icon: '📋',
      avatar: '👋',
      speech: '<strong>Benvenuto! I\'m Yohannes.</strong> When I was selected for UNICORE, I felt overjoyed—and also had a hundred questions: <em>"What should I pack? What paperwork do I need before boarding? Will I miss my home food?"</em> Let me share what I wish someone had told me before I boarded the flight. <strong>Let\'s get you ready for Italy! 🇮🇹</strong>',
      body: `
    ...
    {
      code: '3',
      name: 'Snodo Burocrazia (Days 3–7)',
      short: 'Documents & Kit',
    ...
    {
      code: '4',
      name: 'Campus Universitario (Days 5–10)',
      short: 'Campus & Email',
    ...
    {
      code: '5',
      name: 'Finanze & Borsa di Studio (Days 8–14)',
      short: 'Budget & CFU',
    ...
    {
      code: '6',
      name: 'Sanità Pubblica SSN (Days 10–15)',
      short: 'Healthcare SSN',
    ...
    {
      code: '7',
      name: 'Corso Italiano (Days 12–20)',
      short: 'Spoken Italian',
    ...
    {
      code: '8',
      name: 'Autonomia Cittadina (Days 15–21)',
      short: 'City Autonomy',
    ...
    {
      code: '10',
      name: 'Capolinea Laurea (Days 25–30)',
      short: 'Graduation Launchpad',
  ...
  function buildRouteStepper() {
    const stepper = document.getElementById('simTrackStepper');
    if (!stepper) return;

    stepper.innerHTML = STATIONS.map((st, idx) => `
      <button type="button" class="sim-stepper-node ${idx === 0 ? 'active' : ''}" data-station="${idx}" title="${st.name}" aria-label="Go to ${st.name}">
        <span class="node-code">${st.code}</span>
        <span class="node-title">${st.short}</span>
      </button>
    `).join('');

    stepper.querySelectorAll('.sim-stepper-node').forEach(btn => {
  ...
  function renderActiveStation(index, isUserAction = true) {
    ...
    // 1. Update Stepper Nodes
    const nodes = document.querySelectorAll('.sim-stepper-node');
    nodes.forEach((node, idx) => {
      node.classList.toggle('active', idx === currentStation);
      node.classList.toggle('visited', visitedStations.has(idx));
    });

    const activeNode = document.querySelector(`.sim-stepper-node[data-station="${currentStation}"]`);
    if (activeNode) {
      activeNode.scrollIntoView({ behavior: 'smooth', block: 'nearest', inline: 'center' });
    }

    // 2. Update Header Status
    const badgeCode = document.getElementById('simBadgeCode');
    const stationLabel = document.getElementById('simStationLabel');
    const progressCount = document.getElementById('simProgressCount');

    if (badgeCode) badgeCode.textContent = data.code;
    if (stationLabel) stationLabel.textContent = data.name;
    if (progressCount) progressCount.textContent = `${visitedStations.size} / ${TOTAL_STATIONS}`;

    // 3. Render Card Viewport
    const stageCard = document.getElementById('simStageCard');
    if (stageCard) {
      stageCard.innerHTML = `
        <div class="sim-hero-banner">
          <div class="sim-hero-icon">${data.icon}</div>
          <div>
            <span class="sim-phase-tag">${data.phase}</span>
            <h2 class="sim-hero-title">${data.name}</h2>
          </div>
        </div>
    ...
    const markExploredBtn = document.getElementById('simMarkExploredBtn');
    ...
    if (continueBtn) {
      if (currentStation === TOTAL_STATIONS - 1) {
        ...
        continueBtn.innerHTML = `Travel to Stop ${nextData.code}: ${nextData.short} <i class="fa-solid fa-arrow-right"></i>`;
        continueBtn.style.background = '';
      }
    }

    if (markExploredBtn) {
      markExploredBtn.innerHTML = '<i class="fa-solid fa-circle-check" style="color: #059669;"></i> Stop Explored';
    }
  ...
  function attachControls() {
    ...
    document.getElementById('simContinueBtn')?.addEventListener('click', () => {
      if (currentStation < TOTAL_STATIONS - 1) {
        renderActiveStation(currentStation + 1, true);
      } else {
        playSound('complete');
  ...
  function initTools() {
    ...
    document.getElementById('copyBudgetBtn')?.addEventListener('click', () => {
      ...
      const planText = [
        ...
        '',
        'Generated from Linea UNICORE Transition Simulator (https://ysmjone-max.github.io/Unicore/first-30-days.html)'
      ].join('\n');
